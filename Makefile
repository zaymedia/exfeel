...
#	ssh -o StrictHostKeyChecking=no ${HOST} -p ${PORT} 'cd api_${BUILD_NUMBER} && echo "DB_NAME=${DB_NAME}" >> .env'
#	ssh -o StrictHostKeyChecking=no ${HOST} -p ${PORT} 'cd api_${BUILD_NUMBER} && docker-compose pull'
#	ssh -o StrictHostKeyChecking=no ${HOST} -p ${PORT} 'cd api_${BUILD_NUMBER} && docker-compose up --build --remove-orphans -d'
#	ssh -o StrictHostKeyChecking=no ${HOST} -p ${PORT} 'rm -f api'
#	ssh -o StrictHostKeyChecking=no ${HOST} -p ${PORT} 'ln -sr api_${BUILD_NUMBER} api'
